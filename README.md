...
### Component 2: The Shell Launcher (`~/.scripts/dev-domain.sh`)

Create the file:

```bash
nano ~/.scripts/dev-domain.sh
```

Paste the Zsh environment configuration script:
...
dev-domain() {
    ...
    local APP_NAME=${1:-$DIR_NAME}
    local SOCKET_PATH="/Users/${USERNAME}/.sockets/${APP_NAME}.sock"
...
```bash
chmod +x ~/.scripts/dev-domain.sh
echo "source ~/.scripts/dev-domain.sh" >> ~/.zshrc
source ~/.zshrc
```
...
4.  **Configure the Local App Repository:** Navigate into your new application's codebase folder and create or open your local development environment file:
    ...
    ```
5.  **Launch Your Project Natively:** Run your development server using the global shell launcher from inside your project folder:
    ```bash
    dev-domain
    ```
    *(Alternatively, start your development server as you normally do with `npm run dev` if you are targeting the raw socket directly).*
