...
const getRealUser = () => {
    if (process.env.SUDO_USER) {
        return {
            ...
            uid: execSync(`id -u "${process.env.SUDO_USER}"`, { encoding: 'utf8' }).trim(),
...
const actions = {
    install() {
        ...
        fs.writeFileSync(PLIST_PATH, generatePlist(), { encoding: 'utf8', mode: 0o644 });
    ...
    start() {
        try {
            execSync(`launchctl bootstrap ${DOMAIN} "${PLIST_PATH}"`, { stdio: 'inherit' });
    ...
    stop() {
        try {
            execSync(`launchctl bootout ${DOMAIN} "${PLIST_PATH}" 2>/dev/null`, { stdio: 'inherit' });
    ...
    status() {
        try {
            const out = execSync(`launchctl list | grep ${LABEL} || true`, { encoding: 'utf8' });
            if (out.trim()) {
...
if (actions[command]) {
    ...
    console.log(`Usage: node scripts/daemon.js [install|start|stop|restart|status|uninstall]`);
}
