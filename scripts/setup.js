...
const getRealUser = () => {
    if (process.env.SUDO_USER) {
        ...
        const uid = execSync(`id -u "${username}"`, { encoding: 'utf8' }).trim();
        return {
            username,
            uid,
...
if (!fs.existsSync(socketsDir)) fs.mkdirSync(socketsDir, { recursive: true, mode: 0o755 });
fs.chmodSync(socketsDir, 0o755);

if (!fs.existsSync(scriptsDir)) fs.mkdirSync(scriptsDir, { recursive: true, mode: 0o755 });
...
try {
    execSync('node scripts/daemon.js install', { cwd: projectDir, stdio: 'inherit' });
    console.log('\n🎉 Setup Complete! Your Project Hypervisor is live at https://home.test');
    console.log('👉 To enable the `phv` command in your current terminal session, run: source ~/.zshrc\n');
} catch (err) {
